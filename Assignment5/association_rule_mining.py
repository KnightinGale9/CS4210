...
#use the gain formula provided right after.
    antecedentsdf = ohe_df[row['consequents']]
    # print(temp.info())
    # print(temp)
    antecedentsdf_sum=antecedentsdf.sum(axis=1)
    # print(temp_sum.value_counts())
    # print(temp_sum.value_counts()[len(row['antecedents'])])
    supportCount = antecedentsdf_sum.value_counts()[len(row['consequents'])]
